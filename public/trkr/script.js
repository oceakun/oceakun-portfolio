...
function saveTasks() {
    ...
    document.querySelectorAll('.column').forEach(column => {
        ...
        column.querySelectorAll('.task-card:not(.placeholder)').forEach(taskCard => {
            const title = taskCard.querySelector('h4')?.textContent || '';
            const descElement = taskCard.querySelector('.task-desc');
            const description = descElement?.dataset.rawMarkdown || descElement?.textContent || '';
            const tagsContainer = taskCard.querySelector('.task-tags');
            const tags = tagsContainer ? [...tagsContainer.querySelectorAll('.tag')].map(t => t.textContent) : [];
            tasks.push({
                id: taskCard.id,
                title: title,
...
function loadTasks() {
    ...
    if (savedTasks) {
        try {
            ...
            tasks.forEach(task => {
                addTaskToColumn(task.title, task.description, task.columnId, task.id, task.tags);
            });
...
function addSampleTasks() {
    addTaskToColumn('Task 1: Review requirements', 'Check the project specifications', 'untouched', null, ['review']);
    addTaskToColumn('Task 2: Fix bug', 'Login issue needs fixing', 'working', null, ['bug', 'urgent']);
    addTaskToColumn('Task 3: Deploy app', 'Production deployment', 'done', null, ['deploy']);
    saveTasks();
}
...
function openTaskModal(taskId) {
    ...
    const title = taskCard.querySelector('h4').textContent;
    const descElement = taskCard.querySelector('.task-desc');
    const description = descElement ? descElement.dataset.rawMarkdown || descElement.textContent : '';
    const tagsContainer = taskCard.querySelector('.task-tags');
    const tags = tagsContainer ? [...tagsContainer.querySelectorAll('.tag')].map(t => t.textContent) : [];
...
function updateTaskCounts() {
    document.querySelectorAll('.column').forEach(column => {
        const count = column.querySelectorAll('.task-card:not(.placeholder)').length;
...
function addTaskToColumn(title, description, columnId, existingId = null, tags = []) {
    const columnContent = document.querySelector(`[data-column="${columnId}"] .column-content`);
    if (!columnContent) {
...
function getDragAfterElement(container, y) {
    ...
    const draggableElements = [...container.querySelectorAll('.task-card:not(.dragging)')].filter(
...
function updateTagsPreview(tags) {
    ...
    previewContainer.innerHTML = tags.map((tag, index) => 
